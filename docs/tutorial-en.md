...
As of `1.3.0`, startup is a bit lighter: command history is loaded lazily on first use (`Ctrl+R`), and config/history directories are created only when wtmux actually needs to write data.
...
In `1.3.0`, the history selector is initialized lazily, so simply launching wtmux no longer loads the history file until you first press `Ctrl+R`.
