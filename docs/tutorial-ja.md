...
`1.3.1` では起動処理も少し軽くなっており、コマンド履歴は `Ctrl+R` を初めて使った時に遅延読み込みされ、設定/履歴ディレクトリも実際に書き込みが必要になるまで作成されません。
...
`1.3.1` では、この履歴セレクタは遅延初期化されるため、通常起動しただけでは履歴ファイルを読み込みません。最初に `Ctrl+R` を押した時点で読み込まれます。
